...
ConsumerGroup.prototype.pauseTopics = function (topics) {
  this.pausedTopics = this.topicPayloads.filter(topicPayload => topics.indexOf(topicPayload.topic) !== -1);
  this.topicPayloads = this.topicPayloads.filter(topicPayload => topics.indexOf(topicPayload.topic) === -1);
};

ConsumerGroup.prototype.resumeTopics = function (topics) {
  if (!this.pausedTopics || !this.pausedTopics.length) return;
  const topicsToResume = this.pausedTopics.filter(pausedTopicPayload => topics.indexOf(pausedTopicPayload.topic) !== -1);
  this.pausedTopics = this.pausedTopics.filter(pausedTopicPayload => topics.indexOf(pausedTopicPayload.topic) === -1);
  this.topicPayloads = this.topicPayloads.concat(topicsToResume);
};
